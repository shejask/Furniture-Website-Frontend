...
const ShopSidebarList: React.FC<Props> = ({ data, productPerPage, dataType }) => {
    ...
    let filteredData = data.filter(product => {
        ...
        if (priceRange.min !== 0 || priceRange.max !== 100) {
            ...
            isPriceRangeMatched = unit >= priceRange.min && unit <= priceRange.max;
        }
    ...
    if (sortOption === 'discountHighToLow') {
        filteredData = sortedData
            .sort((a, b) => (
                (Math.floor(100 - ((((b as any).salePrice ?? b.price) / b.originPrice) * 100))) - (Math.floor(100 - ((((a as any).salePrice ?? a.price) / a.originPrice) * 100)))
            ))

    }

    if (sortOption === 'priceHighToLow') {
        filteredData = sortedData.sort((a, b) => (((b as any).salePrice ?? b.price) - ((a as any).salePrice ?? a.price)))
    }

    if (sortOption === 'priceLowToHigh') {
        filteredData = sortedData.sort((a, b) => (((a as any).salePrice ?? a.price) - ((b as any).salePrice ?? b.price)))
    }
                        ...
                        <div className="list-product-block lg:w-3/4 md:w-2/3 w-full md:pl-3">
                            ...
                            <div className="list-product hide-product-sold flex flex-col gap-8 mt-7">
                                ...
                                item.id === 'no-data' ? (
                                    ...
                                    <Product key={item.id} data={item} type='list' style='style-1' />
                                )
                            ))}
                            </div>
