...
const Default: React.FC<Props> = ({ data, productId }) => {
    ...
    useEffect(() => {
        // Fetch category data
        if (productMain?.category && productMain.category.length > 0) {
            fetchCategoryData(productMain.category[0]);
        }
        
        // Fetch brand data
        if (productMain?.brand && productMain.brand.length > 0) {
            fetchBrandData(productMain.brand[0]);
        }
    ...
    }, [productMain?.category, productMain?.brand, productMain?.id]);
